namespace Sipcon.WebApp.Client.Repository
{
    ...
    public class MaintenanceRepository(HttpClient http) : IMaintenanceService
    {
        ...
        public async Task<ApiResponse<Maintenance>> GetMaintenance(int IdUser, int IdDealer, int IdMaintenance) 
        {
            ...
            try
            {
                result = await _http.GetFromJsonAsync<ApiResponse<Maintenance>>($"api/Service/GetOne?userId={IdUser}&serviceTypeId=1&dealerId={IdDealer}&serviceId={IdMaintenance}");

                result = (result is null) ? new ApiResponse<Maintenance>()
                {
                    Processed = false,
                    Message = "La respuesta del servidor no contiene datos."
